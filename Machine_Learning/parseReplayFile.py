...
def parseReplayFile(filename):
    ...
    return [gameInformation, unitInformation,otherGameplayInformation]
...
def parseRoundDelta(root, unitInformation, otherGameplayInformation):
    ...
    currentRound = {}
    #gamePlayInfo can be extended to include other dictionaries of info
    gamePlayInfo = {'resources':{}}
    if lastRoundIndex != -1:
    ...
    for child in children:
        ...
        if childTag=='sig.SpawnSignal':
            ...
            continue
        elif childTag=='sig.TeamResourceSignal':
            parseTeamResourceSignal(child,gamePlayInfo)
    unitInformation[currentMatchIndex].append(currentRound)
    otherGameplayInformation[currentMatchIndex].append(gamePlayInfo)
...
#PARENT METHOD: parseRoundDelta
def parseTeamResourceSignal(root,gamePlayInfo):
    if root.tag != 'sig.TeamResourceSignal':
        return
    resourceSignal = root.attrib
    gamePlayInfo['resources'][resourceSignal['team']]=resourceSignal['resource']
