#SIMILAR TO LEARNING.PY EXCEPT INSTEAD OF NUM_STATES INPUT LAYER AND
#NUM_ACTIONS OUTPUT LAYER, USE NUM_STATES+NUM_ACTIONS INPUT AND ONE Q VALUE OUTPUT
...
GAMMA = 0.5
session = tf.Session()
state = tf.placeholder("float",[None, util.NUM_STATES+util.NUM_ACTIONS])
targets = tf.placeholder("float",[None, 1])
hidden_weights = tf.Variable(tf.constant(0., shape=[util.NUM_STATES+util.NUM_ACTIONS,1]))
