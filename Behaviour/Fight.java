...
public class Fight {
	...
	public static RobotInfo[] spotNearbyTurrets() {
		if (seenAllies == null){return null;}
		int nTurrets = 0;
     ...
     */
	public static boolean locationUnderThreat(RobotInfo[] units, MapLocation loc) {
     ...
     * @return				Returns the robot info of the last targeted or null
     */
    public static RobotInfo targetLastTargeted(RobotInfo[] listOfEnemies) {
        if(lastTargeted == null) {
            return null;
        }
        for(RobotInfo i : listOfEnemies) {
	...
	public static RobotInfo findLowestHealthUninfectedEnemy(RobotInfo[] listOfEnemies) {
		...
		for(RobotInfo i : listOfEnemies) {
			if(i.viperInfectedTurns == 0 && i.health < minHealth) {
				minHealth = i.health;
				bestTarget = i;
			}
		}
