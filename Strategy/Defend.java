...
public class Defend {
    ...
    static int ELECTION = 656565;
    static int MOVE_X = 5341992;
    static int MOVE_Y = 7782;
    static final int PATIENCE = 75;
	...
	static RobotType[] buildList = new RobotType[]{RobotType.SOLDIER,RobotType.SOLDIER, RobotType.SOLDIER, RobotType.TURRET,RobotType.SCOUT};
	static int current_build = 0;
	static int turretNumber = 0;
    static Random rnd;

	static int PERIMETER = 10;
	...
	private void buildSoldierRing() throws GameActionException {
		...
		RobotInfo[] nearby = rc.senseNearbyRobots(PERIMETER+10, rc.getTeam());
		...
		for(int i = 0; i< nearby.length;i++){
			if(nearby[i].type == RobotType.SCOUT){
				scout_here = true;
				break;
			}
		}
		...
		}else if(robotType == RobotType.SCOUT && scout_here){
			robotType = RobotType.SOLDIER;
		}
